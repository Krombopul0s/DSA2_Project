...
def packageStatus():
    ...
    if prompt == 2:
        ...
        startTimeFrame = datetime.strptime(str(input('Please enter the start time (HH:MM): ')), '%H:%M')
        endTimeFrame = datetime.strptime(str(input('Please enter the end time (HH:MM): ')), '%H:%M')
        for truck in trucks:
